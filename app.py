...
with st.form("lumina_v_clean"):
    ...
    # ปรับตรงนี้ให้ดูเป็นการเลือกด้านที่จะคุยครับ
    ...
    # ปรับตรงนี้ให้ดูเป็นการถามถึงรายละเอียดในด้านนั้นๆ
    st.markdown(f"**แชร์รายละเอียดเรื่องที่ติดค้างในใจให้ฟังหน่อยนะคะ...**")
    question = st.text_area("", placeholder="เช่น อกหักมูฟออนไม่ได้, อยากหาทางออกเรื่องงาน...", height=120)
...
if submitted:
    if len(name.strip()) < 2 or len(contact.strip()) < 3 or len(question.strip()) < 5:
        st.error("⚠️ รบกวนระบุข้อมูลให้ครบถ้วน เพื่อให้อุ้มสื่อสารกับพลังงานของคุณได้แม่นยำนะคะ")
    else:
        st.balloons()
        idx = (date % 3)
        
        # (คำตอบคุณภาพแบบเดิมที่คุณอุ้มชอบ)
